...
void handle_umode_exception(unsigned int cause, struct trap_frame* tfr) { 
    // Input: unsigned int cause 
    //        struct trap_frame* tfr
    // Output: n/a
    // Description: handles exceptions in user mode based on cause code. handles system calls and lazy page allocation, 
    //              exits processes under all other cases 
    // Side Effects: may cause current process to exit
    
    // kprintf("in umode exception\n");
    // kprintf("cause: %u \n", cause);
    // kprintf("trap frame addr: %x \n", tfr);
    // kprintf("trap frame \n a0: %x \n a1: %x \n a2: %x \n fp: %x \n sepc: %x \n sstatus: %x \n sp: \n", 
    //     tfr->a0, tfr->a1, tfr->a2, tfr->fp, tfr->sepc, tfr->sstatus, tfr->sp);
    
    const char* name = NULL; 
    char msgbuf[80];

    if (0 <= cause && cause < sizeof(excp_names) / sizeof(excp_names[0])) name = excp_names[cause];     // fill name with exception name based on cause
    ...
    if (name != NULL) {
        switch(cause) {
            ...
            case RISCV_SCAUSE_ECALL_FROM_UMODE:
                // user calls ecall 
                // system call from user mode, resumes function in user mode
                // put syscall number into a7 register

                // place arguments to system call into a0...a6
                // call syscall_handler with trap frame pointer 
                handle_syscall(tfr);
                return;
            case RISCV_SCAUSE_INSTR_PAGE_FAULT:
            case RISCV_SCAUSE_LOAD_PAGE_FAULT: 
                // lazy allocation
                // stval holds location of where page fault occurred
                if (handle_umode_page_fault(tfr, (uintptr_t) csrr_stval()) == 1){
                    // kprintf("allocated to vma: %x \n", (uintptr_t) csrr_stval());
                    return;    // handle/map was successful
                }
                else{
                    panic("any word");
                }
                // if map/handle was not successful 
                snprintf(msgbuf, sizeof(msgbuf), "%s at %p for %p in U mode", name,
                         (void*)tfr->sepc, (void*)csrr_stval());
                break;   
            case RISCV_SCAUSE_STORE_PAGE_FAULT:
