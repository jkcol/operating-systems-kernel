...
smode_trap_entry_from_umode:
        # Inputs: n/a, sp set from trap_frame_entry
        # Outputs: n/a
        # Description: handles a trap entry from umode, sets up trap frame & sets crs 
        # Side Effects: swtich sswitch register and pc
        
        # switches to kernel stack 
        # user kernel _sp_, _tp_, and _gp_ 
        # recover sp from sscratch? write current (user) sp to sscratch & 
        # load kernel sp from scratch into sp  

        # Save the sscratch value and clear sscratch
        csrr t6, sscratch
        sd t6, SP(sp)
        csrw sscratch, 0

        # store all general purpose registers, ra, fp
        sd      a0, A0(sp)
        sd      a1, A1(sp)
        sd      a2, A2(sp)
        sd      a3, A3(sp)
        ...
        csrr    t6, sstatus
        sd      t6, SSTATUS(sp) # load sstatus register; tfr->sstatus = RISCV_SSTATUS_SPIE;
        csrr    t6, sepc
        sd      t6, SEPC(sp) # load sepc register; tfr->sepc = entry;

        # Set up _fp_ to look like a normal stack frame

        addi    fp, sp, TFRSZ

        call umode_trap_entry_cont
        ...
        sret    # done! Return to umode 

umode_trap_entry_cont:
        
        # load scause register into a0 
        csrr    a0, scause 
        # load trap_frame into a1 
        mv a1, sp

        # call handle_umode_exception(unsigned int cause, struct trap_frame* tfr)
        # a0 - unsigned int cause 
        # a1 - struct trap_frame* tfr 
