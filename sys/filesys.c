 ...
 */
 ...
 // Inputs: char* path - string to be parsed, char** mpnameptr - pointer to which the mount point name will be stored, char** flnameptr - pointer to which the file name will be stored
 // Outputs: int - 0 on success, -EINVAL on invalid arguments.
 // Description: parses a path into mount point name and file name
 // Side Effects: modifies path string by inserting null terminators
int parse_path(char* path, char** mpnameptr, char** flnameptr) {
    char *p, *slash; // p: current position in path, slash: position of next slash
    if (!path || !mpnameptr || !flnameptr) return -EINVAL; // invalid arguments
    *mpnameptr = NULL;
    *flnameptr = NULL;
    p = path;
    while (*p == '/') p++; // skip leading slashes
    if (*p == '\0') return 0; // empty path
    slash = strchr(p, '/'); // find next slash
    if (!slash) { // no slash found, only mount point name
        *mpnameptr = p;
        return 0;
    }
    *slash = '\0';
    *mpnameptr = p;
    p = slash + 1;
    if (*p == '\0') return 0; // no file name
    if (strchr(p, '/')) return -EINVAL; // invalid path with multiple slashes in file name
    *flnameptr = p;
    return 0;
}
